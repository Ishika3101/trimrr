...
import {Copy,Trash, LinkIcon, Download } from "lucide-react";
...
const  LinkCard= ({url,fetchUrls}) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-5 border p-4 bg-gray-900 rounded-1g">
      ...
      <Link to={`/link/${url?.id}`} className="flex flex-col flex-1"> 
        ...
        <span className="flex items-end font-extralight text-sm flex-1">{new Date(url?.created_at).toLocaleDateString()}</span>
      </Link>
